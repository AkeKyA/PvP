...
class TeamBuildUHC extends PluginBase implements Listener{
	public $prefix = "§l§b[§6TeamPvP§b]§r ";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
	public $reds = [];
			...
			if($event->getEntity() instanceof Player && $event->getDamager() instanceof Player){
				...
				if($config->get($level . "PlayTime") != null){
					if($config->get($level . "PlayTime") > 570){
						$event->setCancelled(true);
					}elseif(isset($this->reds[$event->getEntity()->getName()]) && isset($this->reds[$event->getDamager()->getName()])){
						$event->setCancelled(true);
	...
	public function onInteract(PlayerInteractEvent $event){
		...
		}elseif($this->mode>=1&&$this->mode<=6){
			$config = new Config($this->getDataFolder() . "/config.yml", Config::YAML);
    ...
    public function onEntityTeleport(EntityTeleportEvent $event){
    	...
    	if($player instanceof Player){
            $to = $event->getTo()->getLevel()->getFolderName();
            $from = $event->getFrom()->getLevel()->getFolderName();
            if(($from !== 'Hub') && ($to == 'Hub')){
            	if(isset($this->reds[$name])){
            		unset($this->reds[$name]);
            	}
            	if(isset($this->blues[$name])){
            		unset($this->blues[$name]);
            	}
            	if(isset($this->greens[$name])){
            		unset($this->greens[$name]);
            	}
            }
    	}
    }
						...
						if(count($playersArena)>=2){
							if($timeToStart>0){
                                        	...
                                        	$config->set($arena . "StartTime", 20);
                                                                ...
                                                                if(($aop>=2))
                                                                    ...
                                                                    foreach($playersArena as $pl)
                                                                        ...
                                                                        {
                                                                        $pla->sendTip("§4RED:" . $reds . "  §1BLUE:" . $blues . "  §aGREEN:" . $greens);
                                                                        }
                                                                }
								$time--;
								if($time == 599)
                                                                ...
                                                                if($time == 450)
								{
									...
									{
										$pl->sendMessage("§e>§e--------------------------------");
                                                                                $pl->sendMessage("§e>§bPlugin made by AkeKy");
                                                                                $pl->sendMessage("§e>§e--------------------------------");
									}
								}
								if($time>=180)
						...
						else
						{
							if($timeToStart<=0)
							{
								foreach($playersArena as $pl)
								{
									foreach($this->plugin->getServer()->getOnlinePlayers() as $plpl)
									{
										$plpl->sendMessage($this->prefix . $pl->getNameTag() . "§ehas won §6TeamPvP! §3Arena: §a" . $arena);
									}
									$spawn = $this->plugin->getServer()->getDefaultLevel()->getSafeSpawn();
									$this->plugin->getServer()->getDefaultLevel()->loadChunk($spawn->getX(), $spawn->getZ());
									$pl->getInventory()->clearAll();
							...
							else
							{
								foreach($playersArena as $pl)
								{
									$pl->sendTip(TextFormat::GREEN . "Need atleast 3 players to start.");
								}
								$config->set($arena . "PlayTime", 600);
								$config->set($arena . "StartTime", 20);
							}
